import os, requests, logging, sys, config, operator, xmltodict, urllib3, sodarr
...
fileHandler = RotatingFileHandler(config.LOG_FOLDER + '/Unmonitor.log', maxBytes=1024 * 1024 * 1, backupCount=1)
		...
		for episode in episodes:
			try:
				if episode['hasFile']:
					...
					filename = original_filename.replace(config.remote_path_local, config.remote_path_remote)
